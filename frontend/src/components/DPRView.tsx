...
export function DPRView({ user, projects, dpr, setDpr, boq, setBoq, showToast }: any) {
  const em = { date: today(), projectId: projects[0]?.id || "", boqId: "", description: "", location: "", category: WORK_CATEGORIES[0], plannedQty: 0, qtyDone: 0, subcon: SUBCONS[0], engineer: user.role === "engineer" ? user.name : ENGINEERS[0], incharge: INCHARGES[0], reason: "", workStatus: "Completed", carryForward: false };
  ...
  const save = () => {
    ...
    const qtyDone = Number(form.qtyDone);
    const pct = plannedQty > 0 ? Math.min(100, Math.round(qtyDone / plannedQty * 100)) : 0;
    const item = boq.find((b: any) => b.id === Number(form.boqId));
    const entry = { ...form, pct, qtyDone, plannedQty, projectId: +form.projectId, boqId: +form.boqId };
    if (editId) setDpr((d: any) => d.map((item: any) => item.id === editId ? { ...item, ...entry } : item));
    else setDpr((d: any) => [{ ...entry, id: uid(), createdAt: Date.now() }, ...d]);
    if (!editId && item && qtyDone > 0) setBoq((b: any) => b.map((it: any) => it.id === Number(form.boqId) ? { ...it, cumDone: Math.min(it.boqQty, it.cumDone + qtyDone) } : it));
    setForm(em);
    setOpen(false);
    showToast(`DPR ${editId ? "updated" : "saved"}! "${item?.item}" BOQ updated.`);
    setEditId(null);
  };
  ...
  return (
    <div className="fadeIn">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
        ...
        {canAdd && <Btn variant="success" onClick={() => { setEditId(null); setForm(em); setOpen(!open); }} icon={open ? "ti-x" : "ti-plus"}>{open ? "Close" : "Add Progress"}</Btn>}
      </div>
      ...
      {open && (
        <Card style={{ borderColor: "#10b981" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 14 }}>
            ...
            <div><Lbl>BOQ Item (auto-updates)</Lbl><Sel value={form.boqId} onChange={(e: any) => { const it = boq.find((b: any) => b.id === +e.target.value); s("boqId", +e.target.value); if (it) { s("description", it.item); s("plannedQty", it.boqQty); } }}>
            ...
            <div><Lbl>Planned Quantity</Lbl><Inp type="number" value={form.plannedQty} onChange={(e: any) => s("plannedQty", e.target.value)} /></div>
            ...
            <div style={{ gridColumn: "span 3", padding: "10px 12px", background: "var(--s2)", borderRadius: 8, color: "var(--t2)", fontSize: 13 }}>Calculated progress: <strong style={{ color: "#10b981", fontSize: 18 }}>{Number(form.plannedQty) > 0 ? Math.min(100, Math.round(Number(form.qtyDone) / Number(form.plannedQty) * 100)) : 0}%</strong></div>
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
            <Btn variant="success" onClick={save} icon="ti-device-floppy">{editId ? "Update DPR" : "Save & Update BOQ"}</Btn>
      ...
      <Card>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 1000 }}>
            <thead><tr>{["#", "Date", "Project", "Description", "Category", "% Done", "Qty Done", "Status", "CF", "Engineer", "Reason", "Edit"].map(h => <TH key={h} c={h} />)}</tr></thead>
            <tbody>
              {dpr.length === 0 && <tr><td colSpan={12} style={{ textAlign: "center", padding: 40, color: "var(--t3)" }}>No DPR entries yet</td></tr>}
