...
export function ProjectsView({ user, projects, setProjects, boq, setBoq, showToast, setActiveProjectId, setMod }: any) {
        ...
        {projects.map((p: any) => {
          ...
          return (
            <div key={p.id} style={{ background: "var(--s1)", border: "1px solid var(--br)", borderRadius: 14, padding: 18, position: "relative", overflow: "hidden" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginTop: 12, paddingTop: 12, borderTop: "1px solid var(--br)", marginBottom: 12 }}>
              ...
              <Btn style={{ width: "100%" }} onClick={() => { setActiveProjectId(p.id); setMod("dwr"); }}>Open Dashboard</Btn>
            </div>
          );
        })}
      </div>
