...
export function Today({ user, projects, dwr, dpr, boq, indents, mat, labour, issues, stockMap, setMod }: any) {
  const td = today();
  ...
  return (
    <div className="fadeIn">
      <div style={{ padding: "22px 26px", borderRadius: 14, background: "linear-gradient(135deg, rgba(251,146,60,0.08), rgba(59,130,246,0.04))", border: "1px solid var(--br)", marginBottom: 20 }}>
      ...
      <div className="dash-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(130px, 1fr))", gap: 12, marginBottom: 20 }}>
        {stats.map(s => (
          <div key={s.lbl} onClick={() => setMod(s.tab)} style={{ background: "var(--s1)", border: "1px solid var(--br)", borderRadius: 13, padding: "16px 14px", cursor: "pointer", position: "relative", overflow: "hidden", transition: "all 0.2s" }}
      ...
      <div className="dash-grid-2" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
        <Card>
          ...
          {projects.map((p: any) => {
            ...
            return (
              <div key={p.id} style={{ marginBottom: 12, paddingBottom: 12, borderBottom: "1px solid var(--br)" }}>
