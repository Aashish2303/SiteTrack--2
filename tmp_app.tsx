...
export default function App() {
  ...
  const NAV = [
    { id: "today", icon: "ti-bolt", label: "Today" },
    { id: "projects", icon: "ti-building-skyscraper", label: "Projects" },
    { id: "dwr", icon: "ti-clipboard", label: "Daily Work" },
    { id: "dpr", icon: "ti-trending-up", label: "Progress" },
    { id: "boq", icon: "ti-list-numbers", label: "BOQ" },
    { id: "materials", icon: "ti-cube", label: "Materials" },
    { id: "indents", icon: "ti-package", label: "Indents" },
    { id: "labour", icon: "ti-users", label: "Labour" },
  ...
  const common = { user, projects, showToast };
  const renderMod = () => {
    ...
    if (mod === "dwr") return <DWRView {...common} dwr={dwr} setDwr={setDwr} />;
    if (mod === "dpr") return <DPRView {...common} dpr={dpr} setDpr={setDpr} boq={boq} setBoq={setBoq} />;
    if (mod === "boq") return <BOQView {...common} boq={boq} setBoq={setBoq} />;
    if (mod === "materials") return <MaterialsView {...common} mat={mat} setMat={setMat} txn={txn} setTxn={setTxn} stockMap={stockMap} />;
    if (mod === "indents") return <IndentsView {...common} indents={indents} setIndents={setIndents} />;
    if (mod === "labour") return <LabourView {...common} labour={labour} setLabour={setLabour} />;
    if (mod === "issues") return <IssuesView {...common} issues={issues} setIssues={setIssues} />;
    ...
    if (mod === "orders") return <MaterialOrders {...common} orders={orders} setOrders={setOrders} vendorDB={vendorDB} />;
    if (mod === "reports") return <QMSReports {...common} dwr={dwr} dpr={dpr} boq={boq} labour={labour} issues={issues} mat={mat} txn={txn} />;
    return null;
  };

  const pendingCount = indents.filter((i: any) => i.status !== "Approved" && i.status !== "Rejected").length;
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh" }}>
      ...
      <aside className={`sidebar ${sidebarOpen ? 'open' : ''}`} style={{ width: 220, background: "var(--s1)", minHeight: "100vh", display: "flex", flexDirection: "column", position: "fixed", left: 0, top: 0, zIndex: 100, borderRight: "1px solid var(--br)" }}>
        ...
        <nav style={{ flex: 1, padding: "8px 0", overflowY: "auto" }}>
          <div style={{ padding: "8px 15px 4px", fontSize: 9, fontWeight: 700, color: "var(--t3)", letterSpacing: "0.12em" }}>MAIN</div>
          {NAV.slice(0, 1).map(navItem)}
          <div style={{ padding: "8px 15px 4px", fontSize: 9, fontWeight: 700, color: "var(--t3)", letterSpacing: "0.12em" }}>SITE MANAGEMENT</div>
          {NAV.slice(1, 9).map(navItem)}
          <div style={{ padding: "8px 15px 4px", fontSize: 9, fontWeight: 700, color: "var(--t3)", letterSpacing: "0.12em" }}>FINANCE & QMS</div>
          {NAV.slice(9).map(navItem)}
        </nav>
